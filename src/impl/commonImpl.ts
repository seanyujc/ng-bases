...
import {IApiConfigProvider} from "../provider";

export const common: IAddMemberFn = function (module: IModule) {
    module.factory('sgCommon', commonFactory);
    function commonFactory($q: ng.IQService, apiConfig: IApiConfigProvider, BARCODE_VRES: IConstBarcodeVres) {
        'ngInject';
        return {
            ...
            // 获取api路径
            dealPath: function (api, type = 'post') {
                if (api === undefined) return '';
                let _api = api, _u = '';
                if (apiConfig[type] === undefined) return '';
            ...
            // 条形码校验
            validateBarcode: function (code): IConstBarcodeVres {
                if (isNaN(code) || code.length !== 13) {
                    return BARCODE_VRES.NOIS;
                }
                let region = parseInt(code.slice(0, 3));
                if ((region >= 690 && region <= 699) || (region >= 977 && region <= 979)) {
                    let c1 = +code.charAt(0) + +code.charAt(2) + +code.charAt(4) + +code.charAt(6) + +code.charAt(8) + +code.charAt(10);
                    let c2 = +code.charAt(1) + +code.charAt(3) + +code.charAt(5) + +code.charAt(7) + +code.charAt(9) + +code.charAt(11);
                    let cc = c1 + c2 * 3;
                    let n = (10 - cc % 10) % 10;
                    if (code.charAt(12) == n) {
